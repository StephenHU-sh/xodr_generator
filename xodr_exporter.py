...
def resample(multiline, d=1.0):
    pts = []
    n = math.ceil(multiline.length / d)
    for idx in range(0, n+1):
        pt = multiline.interpolate(idx/n, normalized=True)
...
def export_road(odr, road, road_id):
    ...
    for idx, (lane_subid, lane) in enumerate(road.items()):
        ...
        right_bnd = MultiLineString([(right_bnd_pts[idx], right_bnd_pts[idx+1]) for idx in range(len(right_bnd_pts)-1)])
        right_bnd_pts = resample(right_bnd, 1.0)
